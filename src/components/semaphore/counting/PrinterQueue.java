package components.semaphore.counting;
...
public class PrinterQueue {
	// This Semaphore will keep track of no. of printers used at any point of
	// time.
	private final Semaphore semaphore;

	// While checking/acquiring a free printer out of three available printers,
	// we will use this lock.
	...
	public void printJob(Object document) {
		try {
			...
			System.out.println(Thread.currentThread().getName() + ": Printer "
					+ assignedPrinter + " : Printing a Job during "
	...
	private int getPrinter() {
		int foundPrinter = -1;
		try {
			// Get a lock here so that only one thread can go beyond this at a
			// time
			printerLock.lock();
